...
- 外部模型候选：机器清单内 `external_model_candidates` 共 6 条，覆盖免费可动 WIP、付费可动开发版、免费静态模型、付费关节展示模型和 1 条待核验线索
...
- Thingiverse 的 **Denton V1** 是免费可动机械候选，页面级许可证为 CC BY 4.0，并称含分组件 STL 与 3D PDF；但同一页面仍明确标注 WIP、头部缺失、STEP 尚未发布。其描述还写明基础 CAD 由 Michael Baddeley 私下发送，因此文件包完整性和衍生再许可权都必须在采用前复核。本项目未下载或镜像该文件包。
- Cults 的 **JRIZZ D-O** 页面列出 Fusion 360 F3Z、机器人端/遥控端 Arduino 草图、STL 和完整硬件思路，是目前最接近机械与控制整合的候选；但它是付费 `CULTS - Private Use` 开发版，页面也明确说仍在开发。购买、接受条款和获取文件必须等待用户明确确认。
- Printables 的 **WF3D 30 cm / 32 文件**版本（CC BY-NC-SA 4.0）与 **CalebTimoteo 11 文件 WIP remix**（CC BY-NC 4.0）都是静态模型；不能替代驱动、平衡和安全结构。
- Gambody 的付费 Personal-use 模型有 116 个 STL、518 mm 高的 life-size 版本、可动头部和发动机/电池改装空间，但没有控制源码、电气设计或平衡算法，不能算作可运行整机包。
- MakerWorld 23859 页面在 2026-07-15 审计时被 Cloudflare 阻断，作者、文件、许可和用途均保持 `UNVERIFIED`。
...
因此，本阶段已经把合法可用的控制与装配资料拉到本地，并把六条外部模型候选按用途和许可分层。结论不再是“网上没有免费 D-O 模型”，而是：**尚未找到一个完整、可公开再分发、同时含机械、控制、电气和真机验证证据的 D-O 整机开源包**。Denton 免费候选仍不完整；JRIZZ/Baddeley/Gambody 都有付费或私用边界；静态模型不能冒充可运行机器人。
