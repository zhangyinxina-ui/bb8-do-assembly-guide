...
阶段16只增加真机遥测与证据审计，不修改几何，也没有新增 `.blend` 检查点。

唯一活动主工程始终位于 `blender/output/BB8_1to1_screen_referenced.blend`。这里的历史检查点仅作阶段证据保留；为遵守“只运行一个Blender工程”的续接规则，后续非几何阶段不再创建新的 `.blend` 副本。
