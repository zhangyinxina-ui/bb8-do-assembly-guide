...
test("server-renders the BB-8 and D-O build guide", async () => {
  ...
  assert.match(html, /D0\/D1舵机与Serial0的软件争用已经形成固定哈希的D22–D25安全变体/);
  assert.match(html, /实体导通、USB\/串口、四路脉宽/);
  assert.match(html, /do_safe_pin_variant_compile\.json/);
  assert.match(html, /do_safe_pin_variant_wiring\.csv/);
  assert.match(html, /build_do_safe_pin_variant\.py/);
  assert.match(html, /修改后源码哈希 f858c485/);
  ...
  assert.match(html, /ERC\/DRC均为0，仍HOLD/);
  assert.match(html, /两层PCB为0项违规、0个未连接项/);
  ...
  assert.match(html, /stage19_kicad_project\.zip/);
  assert.match(html, /BB8_stage19_dual_permissive_gate_schematic\.pdf/);
  assert.match(html, /stage19_kicad_erc\.json/);
  assert.match(html, /stage19_kicad_verification\.json/);
  assert.match(html, /verify_stage19_kicad\.py/);
  assert.match(html, /stage19_dual_permissive_gate\.kicad_pcb/);
  assert.match(html, /stage19_kicad_pcb_drc\.json/);
  assert.match(html, /stage19_kicad_pcb_verification\.json/);
  assert.match(html, /generate_stage19_kicad_pcb\.py/);
  assert.match(html, /verify_stage19_kicad_pcb\.py/);
  assert.match(html, /export_stage19_kicad_pcb\.py/);
  assert.match(html, /BB8_stage19_gate_pcb_isometric\.png/);
  assert.match(html, /1614段走线、54个过孔/);
  assert.match(html, /当前桅杆是Ø24 × 340 mm/);
  assert.match(html, /旧阶段2的Ø12\/Ø8 × 300 mm结论已明确标为不再代表当前模型/);
  assert.match(html, /轮—壳接触仍缺5\.5 mm径向调节预算/);
  assert.match(html, /HOLD_JOINT_TOLERANCE_MATERIAL_AND_PHYSICAL_VALIDATION_REQUIRED/);
  assert.match(html, /BB8_阶段20_结构载荷与公差门\.md/);
  assert.match(html, /BB8_stage20_structural_load_and_tolerance_gate\.md/);
  assert.match(html, /stage20_structural_load_contract\.json/);
  assert.match(html, /stage20_structural_load_results\.json/);
  assert.match(html, /stage20_structural_load_sweep\.csv/);
  assert.match(html, /verify_stage20_structural_load_path\.py/);
...
test("server-renders the complete English build guide", async () => {
  ...
  assert.match(html, /Zero ERC\/DRC, still HOLD/);
  assert.match(html, /two-layer PCB has zero violations and zero unconnected items/);
  ...
  assert.match(html, /Complete KiCad reference project/);
  assert.match(html, /Zero-violation ERC report/);
  assert.match(html, /KiCad schematic cross-audit/);
  assert.match(html, /Schematic verifier/);
  assert.match(html, /Routed two-layer PCB/);
  assert.match(html, /PCB DRC report/);
  assert.match(html, /PCB structural audit/);
  assert.match(html, /PCB generator/);
  assert.match(html, /PCB verifier/);
  assert.match(html, /BB8_stage19_gate_pcb_isometric\.png/);
  assert.match(html, /1,614 track segments, 54 vias/);
  assert.match(html, /mast is a Ø24 × 340 mm envelope/);
  assert.match(html, /legacy Ø12\/Ø8 × 300 mm Stage-2 result is explicitly not current-model evidence/);
  assert.match(html, /5\.5 mm wheel-to-shell radial-adjustment shortfall/);
  assert.match(html, /Stage 20 structural report/);
  assert.match(html, /Stage 20 machine contract/);
  assert.match(html, /Stage 20 HOLD result/);
  assert.match(html, /Structural load sweep/);
  assert.match(html, /Stage 20 verifier/);
  ...
  assert.match(html, /D0\/D1 servo-versus-Serial0 software collision now has a hash-locked D22–D25 variant/);
  assert.match(html, /continuity, USB\/Serial0, four-servo pulses/);
  assert.match(html, /do_safe_pin_variant_compile\.json/);
  assert.match(html, /do_safe_pin_variant_wiring\.csv/);
  assert.match(html, /build_do_safe_pin_variant\.py/);
  assert.match(html, /transformed-source hash is f858c485/);
