...
test("server-renders the BB-8 and D-O build guide", async () => {
  ...
  assert.match(html, /旧审计把它按96 mm球体估算/);
  assert.match(html, /实际轮—壳间隙5\.070 mm/);
  assert.match(html, /轮轴相对球壳切平面偏48\.800°/);
  assert.match(html, /5M-280-15候选同步带/);
  assert.match(html, /12 mm M6×1预压滑台/);
...
test("server-renders the complete English build guide", async () => {
  ...
  assert.match(html, /old formula overstates support by 5\.072 mm/);
  assert.match(html, /5\.070 mm shell gap/);
  assert.match(html, /48\.800° axle-to-tangent error/);
  assert.match(html, /5M-280-15 candidate belt/);
  assert.match(html, /12 mm M6×1 preload slide/);
  assert.match(html, /BB8_stage21_wheel_preload_adjuster\.png/);
