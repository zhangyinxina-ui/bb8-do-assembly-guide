...
test("server-renders the BB-8 and D-O build guide", async () => {
  ...
  assert.match(html, /ERC 0项违规，仍HOLD/);
  assert.match(html, /34个器件引用、91条规范引脚连接、21个网络交叉审计通过/);
...
test("server-renders the complete English build guide", async () => {
  ...
  assert.match(html, /Zero ERC violations, still HOLD/);
  assert.match(html, /34 references, 91 canonical pin connections and 21 nets cross-audit cleanly/);
  ...
  assert.match(html, /Complete KiCad schematic project/);
  assert.match(html, /Zero-violation ERC report/);
  assert.match(html, /KiCad cross-audit result/);
  assert.match(html, /KiCad verifier/);
