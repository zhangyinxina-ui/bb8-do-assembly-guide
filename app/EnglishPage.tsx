  ...
  return (
    <main>
      ...
      <section className="section firmware-section" id="control">
        <div className="section-head"><span>03 / MOTION CONTROLLER</span><h2>It must move—<br />and it must stop.</h2><p>The C++ core and ESP32-S3 adapter compile with quadrature encoders, MPU6050 and dual INA226 current sensing. MDD20A PWM-low means brake rather than isolation. Stage 19 now freezes the pre-CAD equation `PWM_OUT = POWER AND PWM_IN AND SAFE_A AND SAFE_B AND ALERT_N`, pin netlist and 64-row truth table. The safety relay and normally-open contactor still provide bus isolation; PCB, powered and running validation remain pending.</p></div>
        <div className="control-grid">
          ...
          <article><span>10</span><h3>64 / 64 logic rows pass, still HOLD</h3><p>Loss of SAFE_A, SAFE_B, ALERT_N or 3.3 V blocks both PWM outputs. The 12 V input current is 5.175 mA and the 0.5 W resistor has 4.006× derating. Without KiCad/Gerber or bench waveforms the status remains HOLD_PCB_CAD_BENCH_AND_SAFETY_VALIDATION_REQUIRED.</p></article>
        </div>
