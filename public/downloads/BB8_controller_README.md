...
7. 两路开漏 ALERT 必须除了进入 GPIO 14，还进入阶段19外部硬件门，直接把MDD20A左右PWM拉低；MDD20A没有独立EN，真正去能仍由安全继电器和常开接触器完成。
...
| 电流 ALERT | GPIO 14 | 双 INA226 开漏并联；同时进入阶段19硬件门的 `ALERT_N` 许可级 |
...
- INA226适配、软件过流/堵转判定和ALERT配置已编译；阶段19已给出ALERT/SAFE_A/B→PWM合同及0项DRC违规、0个未连接项的两层PCB参考设计，但实体2 mΩ分流、Kelvin布线、PCB独立复核与制造发布、驱动器硬限流和急停回路仍未验证。
